.PHONY: all test run web git clean install test-extended venv help train
...
test:
	$(VENV) && python course-eval-hub-web/src/predict.py


train:
	$(VENV) && cd course-eval-hub-web && python src/train.py
...
web:
	cd course-eval-hub-web && yarn dev
...
install:
	...
	cd course-eval-hub-web && yarn install
